...
static inline lua_Integer posrelat (lua_Integer pos, size_t len) {
  return (pos >= 0) ? pos : (lua_Integer)len + pos;
}
...
static int str_find_aux (lua_State *L, int find) {
  ...
  if (init > (lua_Integer)ls) {  /* start after string's end? */
...
static int str_unpack (lua_State *L) {
  ...
  lua_Integer posi = posrelat(luaL_optinteger(L, 3, 0), ld);
